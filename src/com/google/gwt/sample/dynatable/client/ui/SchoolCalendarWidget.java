...
import com.google.gwt.sample.dynatable.client.SchoolCalendarServiceAsync;
...
public class SchoolCalendarWidget extends Composite {
		...
		public void updateRowData(final int startRow, final int maxRows,
			...
			if (startRow == lastStartRow) {
				if (maxRows == lastMaxRows) {
					...
					//
					Person[] tmpPA = new Person[lastPeople.size()];
					for (int i = 0; i < lastPeople.size(); i++) {
						tmpPA[i] = lastPeople.get(i);
					}
					pushResults(acceptor, startRow, tmpPA);
					return;
				}
			}
			...
			//
			calService.getPeoplesList(startRow, maxRows,
	...
	protected void setVisibleRows(int visibleRows) {
		dynaTable.setStartRow(0);
		dynaTable.setRowCount(visibleRows);
		if (pendingRefresh == null) {
			pendingRefresh = new Command() {
				public void execute() {
					pendingRefresh = null;
